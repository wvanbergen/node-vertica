...
Connection = (function(_super) {
  ...
  Connection.prototype.connect = function(callback) {
    var connect, initialErrorHandler;
    ...
    connect = (function(_this) {
      return function() {
        var redirectConn;
        redirectConn = function(msg) {
          if (_this.debug) {
            console.log('Redirect to = ', msg.host, _this.connectionOptions.port);
          }
          _this.connection.destroy();
          _this.redirctedHost = msg.host;
          _this.connection = net.createConnection(_this.connectionOptions.port, msg.host);
          _this.connection.on('error', initialErrorHandler);
          return _this.connection.on('connect', function() {
            return _this.emit('connect');
          });
        };
        _this.connection = net.createConnection(_this.connectionOptions.port, _this.connectionOptions.host);
        _this.connection.on('error', initialErrorHandler);
        return _this.connection.on('connect', function() {
          if (_this.connectionOptions.loadBalance) {
            _this.connection.removeListener('error', initialErrorHandler);
            _this._bindEventListeners();
            _this.connection.once('data', function(buffer) {
              var message, size;
              if (buffer.toString('ascii') === 'N') {
                if (_this.debug) {
                  console.log("Connection load balancing is turned OFF on server");
                }
                return _this.emit('connect');
              } else {
                size = buffer.readUInt32BE(1);
                message = BackendMessage.fromBuffer(buffer.slice(0, size + 1));
                return redirectConn(message);
              }
            });
            return _this._writeMessage(new FrontendMessage.ConnectionLoadBalance());
          } else {
            return _this.emit('connect');
          }
        });
