...
Connection = (function(_super) {
  ...
  Connection.prototype.connect = function(callback) {
    var connectionStatusReported;
    connectionStatusReported = false;
    this._connectedCallback = (function(_this) {
      return function(err) {
        ...
        return connectionStatusReported = true;
      };
    })(this);
    this.connection = net.createConnection(this.connectionOptions.port, this.connectionOptions.host);
    this.connection.on('error', this._connectedCallback);
    this.connection.on('close', this._connectedCallback);
    this.connection.on('timeout', this._connectedCallback);
            ...
            if ('S' === buffer.toString('utf-8')) {
              ...
              return conn = require('./starttls')(_this.connection, sslOptions, function() {
                ...
                if (!conn.authorized && _this.connectionOptions.ssl === 'verified') {
                  ...
                  return _this._connectedCallback(err);
                } else {
                  if (!conn.authorized) {
              ...
              return _this._connectedCallback(new errors.SSLError("The server does not support SSL connection"));
            }
          });
        } else {
          return _this._handshake();
  ...
  Connection.prototype._handshake = function() {
    ...
    this.connection.removeListener('error', this._connectedCallback);
    this.connection.removeListener('close', this._connectedCallback);
    this.connection.removeListener('timeout', this._connectedCallback);
  ...
  Connection.prototype._initializeConnection = function() {
    ...
    for (_i = 0, _len = initializers.length; _i < _len; _i++) {
      ...
      chain = initializer.bind(this, chain, this._connectedCallback.bind(this));
    }
    return chain();
  };
  ...
  Connection.prototype._initializationSuccess = function() {
    ...
    return this._connectedCallback();
  };
  ...
  Connection.prototype._writeMessage = function(msg) {
    var e;
    if (this.debug) {
      console.log('=>', msg.constructor.name, msg);
    }
    try {
      return this.connection.write(msg.toBuffer(), null);
    } catch (_error) {
      e = _error;
      return this.connection.emit('error', e);
    }
  };
