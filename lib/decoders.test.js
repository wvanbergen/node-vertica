...
describe("string decoders", () => {
    test("should decode type timestamp with timezone", () => {
        // Negative timezone
        const negativeTimezone = Buffer.from([50, 48, 49, 49, 45, 48, 56, 45, 50, 57, 32, 49, 55, 58, 51, 57, 58, 52, 53, 46, 54, 54, 53, 48, 53, 49, 45, 48, 50, 58, 51, 48])
        expect(decoders.string.timestamp(negativeTimezone)).toEqual(new Date(Date.UTC(2011, 7, 29, 20, 9, 45, 665)))

        // Positive timezone
        const positiveTimezone = Buffer.from([50, 48, 49, 49, 45, 48, 56, 45, 50, 57, 32, 50, 50, 58, 49, 50, 58, 52, 48, 46, 48, 51, 50, 50, 43, 48, 50])
        expect(decoders.string.timestamp(positiveTimezone)).toEqual(new Date(Date.UTC(2011, 7, 29, 20, 12, 40, 32)))

        // UTC = no offset
        const utc = Buffer.from([50, 48, 49, 49, 45, 48, 56, 45, 50, 57, 32, 50, 48, 58, 49, 52, 58, 52, 53, 46, 54, 55, 49, 52, 52, 55, 43, 48, 48])
    ...
    test("should decode type timestamp without timezone", () => {
        // Use UTC by default
        const data = Buffer.from([50, 48, 49, 49, 45, 48, 56, 45, 50, 57, 32, 49, 55, 58, 51, 52, 58, 52, 48, 46, 53, 52, 54, 54, 48, 53])
    ...
    test("should decode type date", () => {
        const data = Buffer.from([50, 48, 49, 49, 45, 48, 56, 45, 50, 57])
        expect(decoders.string.date(data)).toEqual(new VerticaDate(2011, 8, 29))
    })

    test("should decode type time", () => {
        const data = Buffer.from([48, 52, 58, 48, 53, 58, 48, 54])
    ...
    test("should decode type boolean", () => {
        expect(decoders.string.boolean(Buffer.from([116]))).toEqual(true)
        expect(decoders.string.boolean(Buffer.from([102]))).toEqual(false)
    })
})