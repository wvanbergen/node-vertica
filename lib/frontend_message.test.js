...
import { Startup, CancelRequest, Describe, Close, Query, Parse, Bind, Flush, Execute, Sync, Terminate, SSLRequest, Password, CopyDone, CopyFail, CopyData } from './frontend_message.js'
import { Authentication } from './authentication.js'

describe("Startup", () => {
  test("should hold the message's information", () => {
    const topic = new Startup('username', 'database')
...
describe("CancelRequest", () => {
  test("should hold the correct information", () => {
    const topic = new CancelRequest(123, 456)
...
describe("Describe", () => {
  describe("Portal", () => {
    test("should hold the correct information", () => {
      const topic = new Describe('portal', 'name')
  ...
  describe("Prepared statement", () => {
    test("should hold the correct information", () => {
      const topic = new Describe('statement', 'name')
...
describe("Close", () => {
  describe("Portal", () => {
    test("should hold the correct information", () => {
      const topic = new Close('portal', 'name')
      expect(topic.type).toEqual(80)
      expect(topic.name).toEqual('name')
    })
  ...
  describe("Prepared statement", () => {
    test("should hold the correct information", () => {
      const topic = new Close('statement', 'name')
      expect(topic.type).toEqual(83)
      expect(topic.name).toEqual('name')
    })
...
describe("Query", () => {
  test("should hold the SQL query", () => {
    const topic = new Query("SELECT * FROM table")
  ...
  test("should encode non-latin UTF-8 strings", () => {
    const topic = new Query("SELECT 'Привет'")
...
describe("Parse", () => {
  test("should hold the name, query and parameter types", () => {
    const topic = new Parse("test", "SELECT * FROM table", [1, 2, 3])
...
describe("Bind", () => {
  test("should hold the portal, prepared statement name and parameter values", () => {
    const topic = new Bind("portal", "prep", ["hello", "world", 123])
...
describe("Execute", () => {
  test("should hold portal name and maximum number of rows", () => {
    const topic = new Execute('portal', 100)
    expect(topic.portal).toEqual('portal')
...
describe("Password", () => {
  test("should encode cleartext password messages correctly", () => {
    const topic = new Password('password')
...
describe("CopyFail", () => {
  test("should format the message correctly", () => {
    const topic = new CopyFail('error')
...
describe("CopyData", () => {
  test("should format the message correctly", () => {
    const topic = new CopyData(Buffer.from('123'))
